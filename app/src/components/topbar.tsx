...
/**
 * TopBar - Floating pill-shaped toolbar overlaying the map.
 *
 * @description Centered floating bar with app branding, metric toggles,
 *              share link, separator, theme/lang controls, and save button.
 *              Matches the modern pill design pattern.
 */
 ...
 *
 * @description Shared across all pages. Center section shows either:
 *              - Default: page nav links (diff-count, diffwinratio)
 *              - Custom: center_controls prop override
...
export default function TopBar({
  ...
  /**
   * Build nav link className with active state.
   *
  ...
  return (
    <div className={`absolute top-4 z-[90] ${align === "left" ? "left-4" : "left-1/2 -translate-x-1/2"}`}>
      <div className="flex items-center gap-1 bg-bg-secondary/95 backdrop-blur-md border border-border-primary rounded-full px-2 py-1.5 shadow-[0_4px_24px_var(--shadow-tooltip)]">
        ...
        <a
          href={`/${locale}`}
          className={`flex items-center gap-2 rounded-full px-4 py-2 transition-colors cursor-pointer group no-underline ${
            is_home
              ? "bg-accent/15"
        ...
        {/* Center controls: custom or default metric toggles */}
        {center_controls ? (
          center_controls
        ) : (
          <>
            {/* Heatmap page link */}
            <a
              href={`/${locale}/diff-count`}
              className={nav_link_class("diff-count")}
              title={t_topbar("heatmap")}
            >
              {/* Heatmap grid icon */}
              <svg
                width="16"
            ...
            {/* DiffWinRatio bubble chart link */}
            <a
              href={`/${locale}/diffwinratio`}
              className={nav_link_class("diffwinratio")}
              title={t_topbar("diffwinratio")}
            >
              {/* Scatter/bubble icon */}
              <svg
                width="16"
              ...
              {t_topbar("diffwinratio")}
            </a>

            {/* Graphs analysis link */}
            <a
              href={`/${locale}/heatmap`}
              className={nav_link_class("heatmap")}
              title={t_topbar("graphs")}
            >
              {/* Bar chart icon */}
              <svg
                width="16"
            ...
            {/* Party analysis link */}
            <a
              href={`/${locale}/party-analysis`}
              className={nav_link_class("party-analysis")}
              title={t_topbar("party_analysis")}
            >
              {/* People/party icon */}
              <svg
                width="16"
              ...
              {t_topbar("party_analysis")}
            </a>
          </>
        )}

        {/* Separator */}
        <div className="w-px h-6 bg-border-primary mx-0.5" />
        ...
        {/* Language switcher (globe icon + name + dropdown) */}
        <div ref={lang_ref} className="relative">
          <button
            ...
            <div className="absolute top-full right-0 mt-2 bg-bg-secondary border border-border-primary rounded-xl shadow-[0_8px_32px_var(--shadow-tooltip)] py-1 min-w-[160px] max-h-[280px] overflow-y-auto z-[100]">
              {LOCALES.map((loc) => {
                ...
                return (
                  ...
                  >
                    <span className="w-5 text-center text-[10px] font-bold uppercase opacity-60">{loc}</span>
                    <span>{meta.name}</span>
                  </button>
                );
              })}
